...
batchUtil.init = function (azureCli) {
  ...
  Object.getPrototypeOf(batch).appendStorageAccountOption = function () {
	this.option('-x, --storage-account <storage-account>', $('the storage account name'));
	this.option('-y, --storage-key <storage-key>', $('the storage account key'));
	this.option('-o, --overwrite', $('if set, existing files will be overwritten'));
	return this;
};
...
batchUtil.createBatchServiceClient = function(options) {
  ...
  return utils.createBatchClient(batchAccountInfo.accountName, batchAccountInfo.accountKey, batchAccountInfo.accountEndpoint);
};
