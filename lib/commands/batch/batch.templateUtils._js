    ...
    if (!__.isUndefined(cmd)) {
      ...
      if (__.isUndefined(isWindows)) {
        ...
        if (isWindows !== cmd.isWindows) {
          throw new Error('The command is not compatible with Windows and Linux.');
        }
      }
    }
  });
...
/**
 * Replace string placeholders with sweep value
 * @param {string} data - The string containing placeholders to be replaced
 * @param {array} parameters - The sweep values, each value maps to one of {0}, {1}, .. {n} by index 
 */
...
batchTemplateUtils.validateJobRequestingApplicationTemplate = function(job, workingFolder, _)
{
  ...
  try {
    fs.access(templateFilePath, fs.R_OK, _);
  } catch (error) {
    throw new Error(util.format('Unable to read the template %s (%s).', templateFilePath, error));      
  }
...
batchTemplateUtils.validateMetadata = function(metadata) {
  ...
  if (!__.isUndefined(violation)) {
    throw new Error(util.format('Metadata item \'%s\' cannot be used; the prefix \'az_batch:\' is reserved for Batch use.', violation.name));
  }
};
