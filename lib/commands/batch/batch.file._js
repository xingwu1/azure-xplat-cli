...
exports.init = function(cli) {
  ...
  */
  ...
  var inputfile = batch.category('file')
    .description($('Commands to manage your Batch input files'));
  ...
  inputfile.command('upload [local-path] [file-group]')
    .description($('Upload a specified file or directory of files to the specified storage path'))
    .option('--local-path <local-path>', $('Path or to a local file or directory to be uploaded can include wildcard patterns'))
    .option('--file-group <file-group>', $('Name of a file group under which the files will be stored'))
    .option('-p, --path <path>', $('Optional group subdirectory under which files will be uploaded'))
    .option('-f, --flatten', $('If set, will not retain local directory structure in storage'))
    .option('-a, --account-name <account-name>', $('the Batch account name'))
    .appendSubscriptionAndResourceGroupOption()
    .execute(uploadInputFiles);
  ...
  */
   ...
   */
  function uploadInputFiles(localPath, cloudPath, options, _) {
    var progress;
    var message = $('Uploading files');
    if (options.accountName) {
      options.resourceGroup = interaction.promptIfNotGiven($('Resource Group: '), options.resourceGroup, _);
    }
    localPath = interaction.promptIfNotGiven($('Local file or directory: '), localPath, _);
    cloudPath = interaction.promptIfNotGiven($('Cloud storage file group name: '), cloudPath, _);

    var references;
    try {
      progress = cli.interaction.progress(message);
      var storageClient = fileUtil.resolveStorageAccount(options, _);
      references = fileUtil.resolveFilePaths(localPath, _); 
       
      if (references.files.length > 0) {
        references.files.forEach_(_, 1, function(_, file) {
          var filename = pathUtil.relative(references.localPath, file);
          fileUtil.uploadBlob(file, cloudPath, filename, options, storageClient, _);
        });
      } else {
        logger.warn(util.format($('No files or directories found matching local path %s'), localPath));
      }
    } finally {
        progress.end();
    }
  ...
  */
  ...
  function getNodeFile(poolId, nodeId, fileName, destination, options, _) {
    ...
    if (cliUtils.fileExists(destination, _)) {
      if (force !== true) {
