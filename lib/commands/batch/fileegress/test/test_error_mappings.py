...
def test_bad_sas():
    ...
    err = azure.common.AzureHttpError(
        u'Server failed to authenticate the request. Make sure the value of Authorization header is formed correctly '
        u'including the signature.\n<?xml version="1.0" encoding="utf-8"?><Error><Code>AuthenticationFailed</Code>'
        u'<Message>Server failed to authenticate the request. Make sure the value of Authorization header is formed '
        u'correctly including the signature.\nRequestId:be9953a6-0001-010c-07ab-2f1267000000\n'
        u'Time:2016-10-26T17:10:28.5908484Z</Message>'
        u'<AuthenticationErrorDetail>Signature fields not well formed.</AuthenticationErrorDetail></Error>', 403)
    agg = uploader.AggregateException([('file', 'pattern', err)])
    error_specification = batchfileuploader.generate_error_specification(agg)
    assert error_specification.code == configuration.ErrorCode.AuthenticationFailed
...
def test_missing_container():
    ...
    err = azure.common.AzureMissingResourceHttpError(
        ...
        u'<Error><Code>ContainerNotFound</Code><Message>The specified container does not exist.\n'
        u'RequestId:ab7b93e5-0001-007b-58b2-2fd173000000\nTime:2016-10-26T17:56:41.7813237Z</Message></Error>', 404)
    agg = uploader.AggregateException([('file', 'pattern', err)])
    error_specification = batchfileuploader.generate_error_specification(agg)
    assert error_specification.code == configuration.ErrorCode.ContainerNotFound
    assert error_specification.file == 'file'
    assert error_specification.pattern == 'pattern'
    assert error_specification.user_error is True
