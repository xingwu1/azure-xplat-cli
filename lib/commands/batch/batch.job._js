...
exports.init = function (cli) {
  ...
  */
  ...
  job.command('create [json-file] [template-file] [parameter-file]')
    ...
    .option('-p, --pool-id <poolId>', $('the id of an existing pool; all the tasks of the job will run on the speicfied pool'))
    .option('--template-file <template-file>', $('the file containing the job object to create in JSON format; if this parameter is specified, all other job parameters are ignored.'))
    .option('--parameter-file <parameter-file>', $('the file containing the job object to create in JSON format; if this parameter is specified, all other job parameters are ignored.'))
  ...
  */
  ...
  function _expandTemplate(templateFile, parameterFile, _) {
    var mergedJobTemplate = {};

    if (templateFile) {
      logger.warn("You are using an experimental feature.");
      // parse the template file
      var templateJsonStr = fs.readFileSync(templateFile).toString();
      var templateJsonObj = JSON.parse(templateJsonStr);

      if (!parameterFile) {
        // TODO: We may need to prompt user for inputs. Look at code in batch.jobUtils which currently throws and error

      } else {
        // parse the parameter file and fill in the blanks
        logger.info("Parsing JSON template and resolving parameters");
        var parameterJsonStr = fs.readFileSync(parameterFile).toString();
        var parameterJsonObj = JSON.parse(parameterJsonStr);
        mergedJobTemplate = templateUtils.parseTemplate(templateJsonStr, templateJsonObj, parameterJsonObj, logger, _);
      }
    }
  ...
  */
  function createJob(jsonFile, templateFile, parameterFile, options, _) {
    var parsedJson = {};

    if (templateFile) {
      var mergedJobTemplate = _expandTemplate(templateFile, parameterFile, _);

      // search for special client-side tags and process accordingly.

      // handle job templates
      // TODO

      // handle task output files
      // TODO

      // bulid up the jsonFile object to hand to the batch service.
      parsedJson = mergedJobTemplate.job.properties;
      parsedJson.id = mergedJobTemplate.job.name;

      // deploy job
      jobUtils.deployJob(parsedJson, options, logger, _);

      // hanlde task factories
      if (parsedJson.taskFactory) {
        var taskCollection = jobUtils.parseTasks(parsedJson.taskFactory, logger, _);
        logger.info(util.format("Deploying %s tasks from task factory", taskCollection.length));
        jobUtils.deployTasks(parsedJson.id, taskCollection, options, logger, _);
      }
    ...
    console.log(util.format($('Job %s has been created successfully'), addJob.id));
  ...
  */
  function showJob(jobId, options, _) {
    ...
    if (options.taskStates) {
      ...
      var counts = jobUtils.getTaskCounts(jobId, options, logger, _);
