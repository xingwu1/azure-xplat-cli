...
batchFileUtils.convertBlobsToResourceFiles = function (blobs, resourceProperties, container, blobService, _) {
  var resourceFiles = [];
  if (blobs.length === 0) {
    throw new Error(util.format("No input data found with reference %s", resourceProperties.source.path));
  }
  if (blobs.length === 1 && blobs[0].name === resourceProperties.source.path) {
...
batchFileUtils.resolveResourceFile = function (resourceFile, options, _) {
  ...
  if (resourceFile.source.fileGroup) {
    ...
    var container = batchFileUtils.getContainerName(resourceFile.source.fileGroup, _);
    var blobs = batchFileUtils.listContainerContents(resourceFile.source.path, container, storageClient, _);
...
batchFileUtils.resolveStorageAccount = function (options, _) {
  ...
  if (!account) {
    ...
    if (resourceGroup) {
      ...
      batchAccount = batchService.account.get(resourceGroup, batchName, _);
    } else {
      ...
      var batchAccounts = batchService.account.list(_).filter_(_, function(_, x){return (x.name === batchName && x.location === region)});
    ...
    batchAccount = batchService.account.get(resourceGroup, account, _);
  } else {
    throw new Error($("No Storage account or Batch account specified"));
  }
  ...
  try {
    ...
    var storageService = cliUtils.createStorageResourceProviderClient(subscriptionOrName);
    var keys = storageService.storageAccounts.listKeys(resourceGroup, account, _);
    connection = { accountName: account, accountKey: keys['keys'][0]['value'] };
 ...
 * @param {callback} _          callback function
 */
batchFileUtils.generateContainerName = function (fileGroup, _) {
 ...
 * @param {callback} _          callback function
 */
batchFileUtils.getContainerName = function (fileGroup, _) {
  return GROUP_PREFIX + batchFileUtils.generateContainerName(fileGroup, _);
};
...
batchFileUtils.uploadBlob = function (sourcefile, destination, filename, options, blobService, _) {
  ...
  var containerName = batchFileUtils.getContainerName(destination, _);
  ...
  var blobType = storage.Constants.BlobConstants.BlobTypes['BLOCK'];
  var storageOptions = storageUtil.getStorageOperationDefaultOption();
  ...
  try {
    ...
    //or is more recent.
    var operation = storageUtil.getStorageOperation(blobService, storageUtil.OperationType.Blob, 'getBlobProperties');
