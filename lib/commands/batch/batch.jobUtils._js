...
batchJobUtils.deployTasks = function (jobId, tasks, options, _) {
  ...
  var client = batchUtil.createBatchServiceClient(options, batchUtil.PREVIEW_USER_AGENT_PREFIX);
  try {
    var start = 0;
...
batchJobUtils.getTargetPool = function (job, options, _) {
  var client = batchUtil.createBatchServiceClient(options, batchUtil.PREVIEW_USER_AGENT_PREFIX);
  ...
  var pool;
  if (!__.isUndefined(job.poolInfo.poolId)) {
    pool = client.pool.get(job.poolInfo.poolId, {}, _);
