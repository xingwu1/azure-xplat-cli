 ...
 */
batchTemplateUtils.validateJobRequestingApplicationTemplate = function(job, _)
{
  ...
  if (__.isUndefined(job.applicationTemplateInfo.filePath)) {
    throw new Error('No filePath specified for requested application template.');
  }
  ...
  try {
    fs.access(job.applicationTemplateInfo.filePath, fs.constants.R_OK, _);
  } catch (error) {
...
batchTemplateUtils.validateApplicationTemplate = function (template)
{
  ...
  if (unsupported.length > 0) {
    throw new Error(util.format('Application templates do not support these properties: %s', unsupported));
  }
 ...
 */
batchTemplateUtils.expandApplicationTemplate = function(job, _) {
  ...
  batchTemplateUtils.validateJobRequestingApplicationTemplate(job, _);

  // Retrieve the application template and expand it with the available parameters
  const templateFilePath = job.applicationTemplateInfo.filePath;
  const templateAsString = fs.readFileSync(templateFilePath).toString();
  const templateAsLoaded = JSON.parse(templateAsString);
