...
exports.init = function(cli) {
  ...
  */
  ...
  pool.command('create [json-file]')
    ...
    .option('--template <template>', $('the file containing the pool object to create in JSON format; if this parameter is specified, all other pool parameters are ignored.'))
    .option('--parameters <parameters>', $('the file containing the pool object to create in JSON format; if this parameter is specified, all other pool parameters are ignored.'))
  ...
  */
  function createPool(jsonFile, options, _) {
    ...
    if (options.template) {
      logger.warn('You are using an experimental feature {Pool Template}.');
      var expandedPoolObject = templateUtils.expandTemplate(cli, options.template, options.parameters, _);

      if (__.isUndefined(expandedPoolObject.pool)) {
        throw new Error('Missing pool element in the template.');
      } 
      if (__.isUndefined(expandedPoolObject.pool.properties)) {
        throw new Error('Missing pool properties element in the template.');
      }       
      // bulid up the jsonFile object to hand to the batch service.
      parsedJson = expandedPoolObject.pool.properties;
    } else if (!jsonFile) {
    ...
    if (!jsonFile && !parsedJson) {
      if (!options.id) {
        ...
        if (!(options.imagePublisher && options.imageOffer && options.imageSku && options.nodeAgentId)) {
          if (options.imagePublisher || options.imageOffer || options.imageSku || options.nodeAgentId) {
            throw new Error($('You must either specify --os-family or all of the folowing: --image-publisher, --image-offer, --image-sku and --node-agent-id.'));
          } else {
    ...
    if (parsedJson.packageReferences) {
      ...
      parsedJson.startTask = templateUtils.constructSetupTask(parsedJson.startTask, cmds);
    }
    ...
    if (!__.isUndefined(parsedJson.clientExtensions) && !__.isUndefined(parsedJson.clientExtensions.dockerOptions)) {
      logger.warn('You are using an experimental feature {Pool creation with Batch Shipyard}.');
      batchShipyardUtils.createPool(parsedJson, options, cli, _);
    ...
    var addPool = client.deserialize(resultMapper, parsedJson, 'result');
