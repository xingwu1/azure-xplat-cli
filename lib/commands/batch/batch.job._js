  ...
  */
  function createJob(jsonFile, options, _) {
    ...
    if (parsedJson.taskFactory) {
      ...
      taskCollection = templateUtils.parseTaskFactory(parsedJson.taskFactory);
    }

    // Handle package management on autopool
    if (parsedJson.poolInfo && parsedJson.poolInfo.autoPoolSpecification && parsedJson.poolInfo.pool) {
      if (parsedJson.poolInfo.pool.packageReferences) {
        logger.warn('You are using an experimental feature {Package Management}.');
        templateUtils.parsePackageReferences(parsedJson.poolInfo.pool);
      }
    }
    ...
    parsedJson = templateUtils.processJobForOutputFiles(parsedJson, taskCollection);
