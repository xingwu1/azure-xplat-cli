...
/**
 * Batch Preview user-agent header
 */
batchUtil.PREVIEW_USER_AGENT_SUFFIX = 'batch-preview:v0.0.1';
...
batchUtil.createBatchManagementClient = function(subscriptionOrName, userAgent) {
  var client;
...
batchUtil.getBatchAccountInformation = function(options) {
  var accountName;
  var accountKey;
  var accountEndpoint;
  ...
  var batchAccountInfo = {};
  batchAccountInfo.accountName = accountName;
  batchAccountInfo.accountKey = accountKey;
  batchAccountInfo.accountEndpoint = accountEndpoint;
  return batchAccountInfo; 
};

batchUtil.createBatchServiceClient = function(options, userAgent) {
  var batchAccountInfo = batchUtil.getBatchAccountInformation(options);
  var client = utils.createBatchClient(batchAccountInfo.accountName, batchAccountInfo.accountKey, batchAccountInfo.accountEndpoint);
  if (userAgent) {
    batchUtil.appendUserAgent(client, userAgent);
  }
  return client;
...
batchUtil.appendUserAgent = function(client, userAgent) {
  var updated = [utils.getUserAgent(), userAgent].join(';');
  var filter = azureCommon.UserAgentFilter.create(updated);
