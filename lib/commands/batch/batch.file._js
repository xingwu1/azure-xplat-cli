...
exports.init = function(cli) {
  ...
  */
  ...
  nodefile.command('delete [pool-id] [node-id] [file-name]')
    ...
    .option('--node-id <node-Id>', $('the id of the compute node from which you want to delete the file'))
    .option('--file-name <user-name>', $('the path to the file that you want to delete'))
  ...
  taskfile.command('delete [job-id] [task-id] [file-name]')
    ...
    .option('--job-id <job-Id>', $('the id of the job that contains the task'))
    .option('--task-id <task-Id>', $('the id of the task whose file you want to delete'))
    .option('--file-name <user-name>', $('the path to the task file that you want to delete'))
    .option('-r, --recursive', $('whether to delete children of a directory'))
    .option('-q, --quiet', $('delete file(s) without confirmation'))
    .appendBatchAccountOption()
  ...
  nodefile.command('download [pool-id] [node-id] [file-name] [destination]')
    ...
    .option('--pool-id <pool-Id>', $('the id of the pool that contains the compute node'))
    .option('--node-id <node-Id>', $('the id of the compute node that contains the file'))
    .option('--file-name <user-name>', $('the path to the file that you want to download'))
  ...
  nodefile.command('show [pool-id] [node-id] [file-name]')
    ...
    .option('--pool-id <pool-Id>', $('the id of the pool that contains the compute node'))
    .option('--node-id <node-Id>', $('the id of the compute node that contains the file'))
    .option('--file-name <user-name>', $('the path to the file that you want to get the properties of'))
    .appendCommonHeaderFilterOption(false, true)
    .appendBatchAccountOption()
  ...
  taskfile.command('show [job-id] [task-id] [file-name]')
    ...
    .option('--job-id <job-Id>', $('the id of the job that contains the task'))
    .option('--task-id <task-Id>', $('the id of the task whose file you want to get the properties of'))
    .option('--file-name <user-name>', $('the path to the file that you want to get the properties of'))
    .appendCommonHeaderFilterOption(false, true)
    .appendBatchAccountOption()
  ...
  nodefile.command('list [pool-id] [node-id]')
    ...
    .option('--pool-id <pool-Id>', $('the id of the pool that contains the compute node'))
    .option('--node-id <node-Id>', $('the id of the compute node whose files you want to list'))
  ...
  taskfile.command('list [job-id] [task-id]')
    ...
    .option('--job-id <job-Id>', $('the id of the job that contains the task'))
    .option('--task-id <task-Id>', $('the id of the task whose files you want to list'))
    .option('-r, --recursive', $('whether to list children of a directory'))
    .appendODataFilterOption(false, true, false)
    .appendBatchAccountOption()
  ...
  */
  ...
  function getNodeFile(poolId, nodeId, fileName, destination, options, _) {
    ...
    var o = {};
    o.timeout = 60000;
    var reader = new ss.ReadableStream(stream, o);
    var fd = fs.openSync(destination, 'w');
    var data;
    do {
      data = reader.read(_, 10000, 60000);
      if (data !== null) {
        fs.write(fd, data, 0, data.length, _);
      }
    } while (data !== null);
    fs.close(fd);
