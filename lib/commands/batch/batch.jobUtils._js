...
function _parseARM(value, template, properties, logger, _) {
    ...
    if (new RegExp("^parameters").test(value)) {
        ...
        if (properties && properties.parameters[subValue]) {
            userValue = properties.parameters[subValue].value;
        }
        if (!userValue) {
...
function _parseParametricSweep(taskFactory, logger, _) {
    ...
    for (var i = start; i <= end; i += skip) {
        ...
        if (taskParams.cmdLine) {
            newTask.commandLine = "cmd /c " + taskParams.cmdLine.replace(/\{\$\}/g, i);
        }
...
// TODO: This is a generic template parser. It should be factored out to its own area
...
batchJob.deployJob = function (ncjId, jobId, properties, options, logger, _) {
    ...
    try {
      client.job.get(jobId, _);
      logger.warn(util.format("Job %s already exists. Continuing to deploy remaining resources.", jobId));
      return false;
    } catch (err) {
      if (!batchUtil.isNotFoundException(err)) {
        if (err.message) {
    ...
    var job = { id: jobId };
    var poolRef = _parsePoolRef(client, properties.pool, _);
