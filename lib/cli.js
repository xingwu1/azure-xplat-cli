...
_.extend(AzureCli.prototype, {
  initSetup: function () {
    ...
    if (!self.debug && process.listeners('uncaughtException').length === 0) {
      self.uncaughtExceptionHandler = function (err) {
      ...
      process.addListener('uncaughtException', self.uncaughtExceptionHandler);
    }
  },
  ...
  enableNestedCommands: function (command) {
    ...
    command.on('*', function () {
      ...
      if (!cat) {
        ...
        var longForm = this._mode === 'arm' ? 'Azure Resource Manager' : 'Azure Service Management';
        var msg = util.format('\'%s\' is not an azure command. See \'azure help\'.\nThe current mode is: %s (%s).', toBlame, this._mode, longForm);
