...
exports.init = function (cli) {
  ...
  */
  function createNcj(jsonTemplate, jsonProperties, options, _) {
    ...
    jsonTemplate = interaction.promptIfNotGiven($('NCJ Template: '), jsonTemplate, _);
    if (!jsonProperties) {
      logger.warn("No properties file was supplied. Using defaults.");
    }
    var strJson = fs.readFileSync(jsonTemplate).toString();
    var parsedJson = JSON.parse(strJson);
    var parsedParams = null;
    if (jsonProperties) {
      parsedParams = JSON.parse(fs.readFile(jsonProperties, _).toString());
    }
    var ncjId = parsedJson.name || uuid.v4();
    ...
    if (ncjParsedJobs) {
      ...
      for (var i = 0; i < ncjParsedJobs.length; i++) {
        ...
        var deployed = jobClient.deployJob(ncjId, jobId, job.properties, options, logger, _);
        if (deployed) {
