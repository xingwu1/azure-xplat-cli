...
exports.init = function (cli) {
  ...
  */
  function createNcj(jsonTemplate, jsonProperties, options, _) {
    ...
    //TODO substitute all parameters and variables at beginning.
    ...
    var ncjParsedPool = parsedJson.resources.filter(function (x) { return x.type === "Microsoft.Batch/batchAccounts/pools" })[0];
    ...
    logger.verbose(JSON.stringify(ncjParsedApplications));

    // Print Pool
    logger.verbose("");
    logger.verbose("Pool");
    logger.verbose(JSON.stringify(ncjParsedPool));

    // Print Job
    logger.verbose("");
    logger.verbose("Job");
    logger.verbose(JSON.stringify(ncjParsedJobs));
