...
exports.init = function(cli) {
  ...
  */
  ...
  pool.command('create [json-file]')
    ...
    .option('--template <template>', $('the file containing the pool as a JSON template; this can only be used with --parameters.'))
    .option('--parameters <parameters>', $('the file containing the pool template parameter values in JSON format; this can only be used with --template.'))
  ...
  */
  function createPool(jsonFile, options, _) {
    ...
    if (options.template) {
      ...
      userAgent = batchUtil.PREVIEW_USER_AGENT;
    ...
    if (userAgent) {
      batchUtil.appendUserAgent(client, userAgent);
    }
