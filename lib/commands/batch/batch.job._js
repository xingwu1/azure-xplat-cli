...
exports.init = function (cli) {
  ...
  */
  ...
  job.command('create [json-file] [template] [parameters]')
    ...
    .option('--template <template>', $('the file containing the job object to create in JSON format; if this parameter is specified, all other job parameters are ignored.'))
    .option('--parameters <parameters>', $('the file containing the job object to create in JSON format; if this parameter is specified, all other job parameters are ignored.'))
  ...
  */
  function createJob(jsonFile, template, parameters, options, _) {
    var parsedJson = {};

    // TODO: Move the actual job deployment bit out, and have a single code base
    // TODO: All pre and post processing steps can happen afterwards.
    if (template) {
      var expandedJobObject = _expandTemplate(template, parameters, _);

      // bulid up the jsonFile object to hand to the batch service.
      parsedJson = expandedJobObject.job.properties;

      // Handle any special pre-processing steps
      ...
      if (parsedJson.taskFactory) {
        // Handle parsing before create job so we can stop when template is wrong
        taskCollection = jobUtils.parseTaskFactory(parsedJson.taskFactory, logger, _);
      }
