...
batchShipyardUtils.invokeBatchShipyard = function(credentialsJson, configJson, poolJson, jobsJson, action, cli, _) {
  ...
  try {
    // Invoke Batch Shipyard
    var command = util.format('python %s %s --configdir %s', batchShipyardPath, action, batchShipyardConfigDir);
...
batchShipyardUtils.createPool = function (poolJson, options, cli, _) {
  var credentialsJson = batchShipyardUtils.createCredentialsJsonFromPool(poolJson, options, cli, _);
  ...
  batchShipyardUtils.invokeBatchShipyard(credentialsJson, configJson, batchShipyardPoolJson, undefined, 'pool add', cli, _);
};
 ...
 * @param {object} pool            the pool referenced by the job
 * @param {object} options         the commandline options
 * @param {object} cli             the cli object
 * @param {callback} _             callback function
 */
batchShipyardUtils.createJobAndAddTasks = function (jobJson, taskCollection, pool, options, cli, _) {
  ...
  var batchShipyardPoolJson = batchShipyardUtils.createBatchShipyardPoolJsonForJob(jobJson, pool);
  ...
  batchShipyardUtils.invokeBatchShipyard(credentialsJson, configJson, batchShipyardPoolJson, batchShipyardJobJson, 'jobs add', cli, _);
};
...
/**
 * Create the content of the Batch Shipyard credentials.json given an expanded pool definition.
 * @param {object} poolJson      the expanded pool definition
 * @param {object} options       the commandline options
 * @param {object} cli           the cli object
 * @param {callback} _           the callback function
 * @returns {object}             the contents of the Batch Shipyard credentials.json file
 */
batchShipyardUtils.createCredentialsJsonFromPool = function (poolJson, options, cli, _) {
  var credentials = batchShipyardUtils.createCredentialsJson(options, cli, _);
  credentials = batchShipyardUtils.addDockerHubLoginToCredentialsConfig(credentials, poolJson);

  return credentials;
};

/**
 * Adds Docker Hub credentials to a Batch Shipyard credentials configuration
 * @param {object} credentialsJson   the base contents of the Batch Shipyard credentials.json file
 * @param {object} poolJson          the expanded pool definition
 * @returns {object}                 the contents of the Batch Shipyard credentials.json file which include Docker Hub login info
 */
batchShipyardUtils.addDockerHubLoginToCredentialsConfig = function (credentialsJson, poolJson) {
  if (!__.isUndefined(poolJson.clientExtensions.dockerOptions.registry)) {
    var dockerRegistry = {};
    if (!__.isUndefined(poolJson.clientExtensions.dockerOptions.registry.hub)) {
      var hub = {};
      if (!__.isUndefined(poolJson.clientExtensions.dockerOptions.registry.hub.username)) {
        hub['username'] = poolJson.clientExtensions.dockerOptions.registry.hub.username;
      }
      if (!__.isUndefined(poolJson.clientExtensions.dockerOptions.registry.hub.password)) {
        hub['password'] = poolJson.clientExtensions.dockerOptions.registry.hub.password;
      }
      dockerRegistry['hub'] = hub;
    }
    credentialsJson.credentials['docker_registry'] = dockerRegistry;
  }
  ...
  if (!__.isUndefined(poolJson.clientExtensions.dockerOptions.registry)) {
    ...
    if (!__.isUndefined(poolJson.clientExtensions.dockerOptions.registry.private)) {
      ...
      if (!__.isUndefined(poolJson.clientExtensions.dockerOptions.registry.private.allowPublicPullOnMissing)) {
        privateOptions['allow_public_docker_hub_pull_on_missing'] = poolJson.clientExtensions.dockerOptions.registry.private.allowPublicPullOnMissing;
      }
      dockerRegistry['private'] = privateOptions;
    }
...
/** 
 * Create the Batch Shipyard pool.json contents from an expanded job definition and its pool
 * @param {object} jobJson       the expanded job definition
 * @param {object} pool          the pool referenced by the job
 * @returns {object}             the contents of the Batch Shipyard pool.json file
 */
batchShipyardUtils.createBatchShipyardPoolJsonForJob = function(jobJson, pool) {
  // Validation that the pool is not an autopool
  ...
  var poolSpec = batchShipyardUtils.createBatchShipyardPoolJson(pool);
  return poolSpec;
};
