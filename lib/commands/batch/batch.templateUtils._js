...
/**
 * Constructs a command line for the job prep task which will run the setup script.
 * @param {object} jobPrepTask    The original job preparation task
 * @returns {object}              An updated job preparation task
 */
batchTemplateUtils.constructJobPrepTask = function (jobPrepTask) {
  var setupCommandLine = 'setup.sh';
  var result;
  if(!__.isUndefined(jobPrepTask)) {
    result = __.extend(jobPrepTask);
  } else {
    result = {};
  }

  if(__.isUndefined(result.commandLine)) {
    result.commandLine = util.format("/bin/bash -c '%s > $AZ_BATCH_JOB_PREP_DIR/uploadsetuplog.txt 2>&1'", setupCommandLine);
  } else {
    // Escape the users command line
    result.commandLine = shellQuote.quote([util.format('%s;%s > $AZ_BATCH_JOB_PREP_DIR/uploadsetuplog.txt 2>&1', result.commandLine, setupCommandLine)]);

    result.commandLine = util.format("/bin/bash -c %s", result.commandLine); 
  }
  ...
  // TODO: For now I put them in a public container but move it to be a GITHUB URL?
  result.resourceFiles.push({ blobSource: 'https://matthchr1.blob.core.windows.net/aaatemp/batchfileuploader.py', filePath: 'batchfileuploader.py' });
  result.resourceFiles.push({ blobSource: 'https://matthchr1.blob.core.windows.net/aaatemp/configuration.py', filePath: 'configuration.py' });
  result.resourceFiles.push({ blobSource: 'https://matthchr1.blob.core.windows.net/aaatemp/requirements.txt', filePath: 'requirements.txt' });
  result.resourceFiles.push({ blobSource: 'https://matthchr1.blob.core.windows.net/aaatemp/setup.sh', filePath: 'setup.sh' });
  result.resourceFiles.push({ blobSource: 'https://matthchr1.blob.core.windows.net/aaatemp/uploadfiles.sh', filePath: 'uploadfiles.sh' });
  result.resourceFiles.push({ blobSource: 'https://matthchr1.blob.core.windows.net/aaatemp/uploader.py', filePath: 'uploader.py' });
  result.resourceFiles.push({ blobSource: 'https://matthchr1.blob.core.windows.net/aaatemp/util.py', filePath: 'util.py' });
...
batchTemplateUtils.parseTaskOutputFiles = function (task) {
  ...
  newTask.commandLine = shellQuote.quote([util.format('%s;%s > $AZ_BATCH_TASK_DIR/uploadlog.txt 2>&1', newTask.commandLine, uploadCommandLine)]);
...
batchTemplateUtils.processJobForOutputFiles = function(job, tasks) {
  ...
  if(mustEditJob) {
    result.jobPreparationTask = batchTemplateUtils.constructJobPrepTask(result.jobPreparationTask);
  }
...
batchTemplateUtils.parseTaskCollectionTaskFactory = function (taskFactory) {
  ...
  __.each(tasks, function(task) {
    ...
    var propertiesToCopy = __.pick(task, 'displayName', 'resourceFiles', 'envrionmentSettings', 'constraints', 'runElevated', 
      'multiInstanceSettings', 'dependsOn', 'exitConditions', 'clientExtensions', 'outputFiles');
...
function parseRepeatTask(task) {
  ...
  var propertiesToCopy = __.pick(task, 'displayName', 'resourceFiles', 'envrionmentSettings', 'constraints', 'runElevated', 
    'exitConditions', 'clientExtensions', 'outputFiles');
  __.extend(newTask, propertiesToCopy);
  return newTask;
}
...
/**
 * Constructs a command line for the start task which will run the setup script.
 * @param {object} startTask    The original pool start task
 * @param {string} commandLine  The additional command line
 * @param {boolean} isWindows   The flag indicate windows environment
 * @returns {object}              An updated start task
 */
batchTemplateUtils.constructStartTask = function (startTask, commandLine, isWindows) {

  var result;
  if(!__.isUndefined(startTask)) {
    result = __.extend(startTask);
  } else {
    result = {};
  }
  ...
  if (isWindows) {
    if(__.isUndefined(result.commandLine)) {
      result.commandLine = util.format('cmd.exe /c "%s"', commandLine);
    } else {
      result.commandLine = util.format('cmd.exe /c "%s & %s"', commandLine, result.commandLine); 
    }
  } else {
    if(__.isUndefined(result.commandLine)) {
      result.commandLine = util.format('/bin/bash -c \'%s\'', commandLine);
    } else {
      result.commandLine = util.format('/bin/bash -c \'%s;%s\'', commandLine, result.commandLine); 
    }
  }
...
/**
 * Parse package reference section in the JSON object 
 * @param {object} request                  Pool object
 */
batchTemplateUtils.parsePackageReferences= function (pool) {
  if (!__.isArray(pool.packageReferences)) {
    throw new Error('PackageReferences has to be a collection.');
  }
  ...
  pool.packageReferences.forEach(function (reference) {

    if (__.isUndefined(reference.type) || __.isUndefined(reference.id)) {
      throw new Error('A PackageReference must have a type or id element.');
    }

    if (reference.type === 'aptPackage') {
      if (isChoco || isYum) {
        throw new Error('PackageReferences can only contain a single type package references.');
      }
      ...
      if (isApt || isYum) {
        throw new Error('PackageReferences can only contain single type package references.');
      }
  ...
  pool.startTask = batchTemplateUtils.constructStartTask(pool.startTask, command, isWindows);  
};
