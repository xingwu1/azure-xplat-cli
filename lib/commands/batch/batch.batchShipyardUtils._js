...
batchShipyardUtils.invokeBatchShipyard = function(credentialsJson, configJson, poolJson, jobsJson, action, cli, _) {
  ...
  batchShipyardPath = cli.interaction.promptIfNotGiven(
    'Please enter the full path to shipyard.py. To avoid seeing this prompt in the future, please set the AZURE_BATCH_SHIPYARD_PATH environment variable: ',
    batchShipyardPath, _);
  ...
  var jobsFile = undefined;
  fs.writeFile(credentialsFile, JSON.stringify(credentialsJson, null, 2), _);
  fs.writeFile(configFile, JSON.stringify(configJson, null, 2), _);
  fs.writeFile(poolFile, JSON.stringify(poolJson, null, 2), _);
  if (!__.isUndefined(jobsJson)) {
    jobsFile = path.join(batchShipyardConfigDir, 'jobs.json');
    fs.writeFile(jobsFile, JSON.stringify(jobsJson, null, 2), _);
  }
...
batchShipyardUtils.getIgnoredTaskPropertyNamesFromFactory = function(taskFactory) {
  var ignoredProperties = [];
  if (!__.isUndefined(taskFactory.tasks)) {
  ...
  if (!__.isUndefined(taskFactory.repeatTask)) {
    ignoredProperties.push(batchShipyardUtils.getIgnoredTaskPropertyNames(taskFactory.repeatTask));
  }
  if (!__.isUndefined(taskFactory.mergeTask)) {
    ignoredProperties.push(batchShipyardUtils.getIgnoredTaskPropertyNames(taskFactory.mergeTask));
  }
